...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    if (available.length === 0) {
      log.info("All agents are already configured.");
      return;
    }
    ...
    if (args.target) {
      ...
      if (current.includes(target)) {
        log.info(`${AGENT_DEFINITIONS.find((d) => d.target === target)!.displayName} is already configured.`);
        return;
      }
    } else {
    ...
    config.targets[target] = true;
    await writeConfig(root, config);
    await generate(root, config);

    log.success(`Added ${AGENT_DEFINITIONS.find((d) => d.target === target)!.displayName}`);
  },
});
